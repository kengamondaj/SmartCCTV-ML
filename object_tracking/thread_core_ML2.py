...
def create_detections(detection_mat, frame_idx, min_height=0):
    ...
    for row in detection_mat[mask]:
        bbox, confidence, feature , f_bbox= row[2:6], row[6], row[15:] ,row[10:14]
...
def golay_filter(df_track, window_size=45, order=5):
    if(len(df_track) <= window_size):
        return df_track
    df_track[1] = savitzky_golay(df_track[1].values, window_size=window_size, order=order, deriv=0, rate=1)
...
def run_showresult(video_path, track_path, feat_path, save_output, out_dir, cap_dir, concat, smoothing, save_fig, is_plot, lag = 30):
    ...
    seq_info = gather_sequence_info_2(video_name, video_path, feat_path)
...
# test_video = 'testr.avi'
list_video = ['0.avi','1.avi','2.avi']
...
# image_tensor_node = detection_graph.get_tensor_by_name('image_tensor:0')
...
def run_process(sess,list_video):
    ...
    file_name = os.path.join(tracks_dir, video_name[:-3]+'csv')
    all_results.to_csv(file_name, header=None)
      

import threading 
l1 = ['test_0-5.avi','test_5-10.avi','test_10-15.avi'] 
l2 = ['test_0-55.avi','test_5-100.avi','test_10-155.avi']
l3 = ['test_0-555.avi','test_5-1000.avi','test_10-1555.avi']

with detection_graph.as_default():
    with tf.Session(graph=detection_graph) as sess:
        for l1_v,l2_v,l3_v in zip(l1,l2,l3):
            print((l1_v,l2_v,l3_v))
            t1 = threading.Thread(target=run_process, args=(sess,[l1_v],)) 
            t2 = threading.Thread(target=run_process, args=(sess,[l2_v],)) 
            t3 = threading.Thread(target=run_process, args=(sess,[l3_v],))
