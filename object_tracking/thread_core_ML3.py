...
#                 ID = str(cam)+'_'+str(ID)
        ...
        visualizer.run(frame_callback)
    f = open('dataset/tracks/'+video_name[:-3]+'csv', 'w')
    for num ,row in enumerate(results):
        print('%d,%d,%.2f,%.2f,%.2f,%.2f,1,-1,-1,-1,%d,%d,%d,%d,-1,%s' % (
            row[0], row[1], row[2], row[3], row[4], row[5] 
            ,row[10], row[11], row[12], row[13], row[15]),file=f)
...
l3 = ['test_0-555.avi','front-509-20200203143657.avi']
with detection_graph.as_default():
